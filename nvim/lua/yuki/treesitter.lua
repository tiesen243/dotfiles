...
M.check = function()
  ...
  ---@class yuki.treesitter.Health: table<string,boolean>
  local ret = {
...
M.build = function(cb)
  M.ensure_treesitter_cli(function(_, err)
    ...
    if ok then
      ...
      vim.notify(table.concat(lines, "\n"), vim.log.levels.ERROR, { title = "YukiVim Treesitter" })
    end
  end)
end

---@param cb fun(ok:boolean, err?:string)
M.ensure_treesitter_cli = function(cb)
